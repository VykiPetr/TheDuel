let startBtn = document.querySelector('.btn-primary')
let nameBox = document.querySelector('.input-group.input-group-sm.mb-3')
let posBtns = document.querySelector('.btn-group')
let topBtn = document.querySelector('.top')
let midBtn = document.querySelector('.mid')
let lowBtn = document.querySelector('.low')
let duelBtn = document.querySelector('.duel')
let riposteBtn = document.querySelector('.riposteBtn')
let contBut = document.querySelector('.continue')
let inputName = document.querySelector('.form-control')
let clearIntervale = document.querySelector('.clearInterval')
let resetBtn = document.querySelector('.reset')
let instrContainer = document.querySelector('#instructions')
let instructionBtn = document.querySelector('.instructions')
let mainCanvasCont = document.querySelector('#container')
let instrBackBtn = document.querySelector('.instrBack')
let buttonContainerMain = document.querySelector('.buttonCont')
var enteredName = ''

console.log(instrBackBtn)


// document.addEventListener('load', () => {
//     splashImg()
// })

